...
//Buttons variables, includes all question elements
...
// Submit, clear, view scores & go back
...
// ARRAY: 5 questions, base starting at 0 
const questions = [ 
    {
...
// Event function to check the answers
function checkAnswer(event) {
    event.preventDefault();

    //Event elements "right" "wrong"
    correctWrong.style.display = "block";
    let p = document.createElement("p");
    correctWrong.appendChild(p);

    // After certain time passes, new element shows
    setTimeout(function () {
        p.style.display = 'none';
    }, 1000);

    // Conditional statement for "Correct"
    if (questions[questionCount].correctAnswer === event.target.value) {
        p.textContent = "Correct!";
    } 
   
     // Conditional statement for "Wrong"
    else if (questions[questionCount].correctAnswer !== event.target.value) {
        secondsLeft = secondsLeft - 10;
        p.textContent = "Wrong!";
    }

    // If statement for question count 
...
// Event listener on click to begin quiz
start.addEventListener("click", startQuiz);

// Event listener on click to check answer
ansBtn.forEach(item => {
    item.addEventListener('click', checkAnswer);
});

// Event listener on click for score
submitScrBtn.addEventListener("click", addScore);

// Event listener on click for "go back"
...
// Event listener on click to "clear"
clearScrBtn.addEventListener("click", clearScores);

// High score event listener alert on click
